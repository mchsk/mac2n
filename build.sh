...
do_install() {
    ...
    if [ -d "$BUILD_DIR/build" ] && [ -f "$BUILD_DIR/build/edge" ]; then
        ...
        # n2n Makefile.in on darwin: SBINDIR=$(PREFIX)/local/sbin with PREFIX=/usr
        # which resolves to /usr/local/sbin (correct). But MANDIR defaults to
        # /usr/share/man which is SIP-protected on modern macOS.
        # Always override MANDIR; override SBINDIR for non-default PREFIX.
        if [ "$PREFIX" = "/usr/local" ]; then
            sudo make install MANDIR="${PREFIX}/share/man"
        else
...
do_bundle_openssl() {
    ...
    for bin in "${PREFIX}/sbin/edge" "${PREFIX}/sbin/supernode"; do
        [ -f "$bin" ] || continue
        sudo install_name_tool -change "$linked_path" "$dest" "$bin" 2>/dev/null || true
    done

    _detail "$dylib_name bundled to $lib_dir"
}
