...
do_uninstall() {
    ...
    if [[ -f /usr/local/sbin/edge ]] || [[ -f /usr/local/sbin/supernode ]]; then
        if prompt_yn "  Remove n2n binaries from /usr/local/sbin? [Y/n]"; then
            local fw="/usr/libexec/ApplicationFirewall/socketfilterfw"
            for bin in /usr/local/sbin/edge /usr/local/sbin/supernode; do
                [[ -f "$bin" ]] || continue
                [[ -x "$fw" ]] && sudo "$fw" --remove "$bin" 2>/dev/null || true
                sudo rm -f "$bin"
            done
            sudo rm -rf /usr/local/lib/n2n 2>/dev/null || true
    ...
    if [[ -L "$LINK_PATH" ]] || [[ -f "$LINK_PATH" ]]; then
        if $remove_dir; then
            sudo rm -f "$LINK_PATH"
            printf "  ${GREEN}✓${RESET} Removed $LINK_PATH\n"
        else
            printf "  ${DIM}…${RESET} Kept $LINK_PATH (install dir was kept)\n"
        fi
    fi
...
ensure_xcode() {
    ...
    sudo touch "$marker"
    _CLEANUP_CMDS+=("sudo rm -f '$marker'")
    ...
    if [[ -n "$label" ]]; then
        ...
        if is_ssh; then
            ...
            _CLEANUP_CMDS=("${_CLEANUP_CMDS[@]/"sudo rm -f '$marker'"}")
            return
        fi
...
build_and_install() {
    ...
    _CLEANUP_CMDS+=("kill $sudo_keepalive_pid")
...
harden_and_verify() {
    ...
    if ! bash ./build.sh verify 2>&1 | sed "s/^/        /"; then
        die $n "Binary verification failed — check: otool -L /usr/local/sbin/edge"
    fi
...
link() {
    ...
    if command -v mac2n &>/dev/null; then
        ...
        local rcfile=".zshrc"
        [[ "$(basename "${SHELL:-/bin/zsh}")" == "bash" ]] && rcfile=".bash_profile"
        printf "        ${YELLOW}Run:${RESET} echo 'export PATH=\"/usr/local/bin:\$PATH\"' >> ~/%s && source ~/%s\n" "$rcfile" "$rcfile"
    fi
}
